...
class MidiMe {
    ...
    async initializeFromFile(filePathEncoder: string, filePathDecoder: string) {
        ...
        this.encoder = await tf.loadLayersModel(filePathEncoder);
        this.decoder = await tf.loadLayersModel(filePathDecoder);

        const x = this.encoder.inputs[0];
        const [z, ,] = this.encoder.apply(x) as tf.SymbolicTensor[];
        const y = this.decoder.apply(z) as tf.SymbolicTensor;
        ...
        logging.logWithDuration('Initialized model from files', startTime, 'MidiMe');
    }

    async saveModel(filePathEncoder: string, filePathDecoder: string) {
        await this.encoder.save(filePathEncoder);
        await this.decoder.save(filePathDecoder);
    }
